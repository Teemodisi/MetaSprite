...
namespace MetaSprite
{
    ...
    public class ASEFile
    {
        ...
        public ASEFile()
        {
            rootGroup.groupIndex = -1;
            rootGroup.name = "Sprites";
            rootGroup.parent = null;
            rootGroup.parentIndex = -2;
            index2Group.Add(rootGroup.groupIndex, rootGroup);
    ...
    public class Layer : UserDataAcceptor
    {
        ...
        public int parentIndex; // =1 if level==0 (have no parent), otherwise the layerIndex of direct parent
        ...
        private LayerParam CheckParamType(int index, LayerParamType type)
        {
            if (parameters.Count <= index)
            {
    ...
    public class Group
    {
        ...
        public bool available => !originName.StartsWith("//");
        public bool haveTarget => destName.Length != 0;
        // public Vector2 offsetPositionFromRoot => groupIndex == -1 ? transformPos : parent.offsetPositionFromRoot + transformPos;
        ...
        public string name
        {
            set
            {
                ...
                m_Name = originName.Substring(nameIndex, targetIndex - nameIndex);
                if (m_Name.Length == 0)
    ...
    {
        private static readonly Color Opaque = new Color(0, 0, 0, 0);
    ...
    public static class ASEParser
    {
                            ...
                            case CHUNK_LAYER:
                                {
                                    ...
                                    if (visible)
                                    {
                                        ...
                                        {
                                            var group = new Group();
                                            group.groupIndex = readLayerIndex;
                                            ...
                                            {
                                                //Add map
                                                file.index2Group.Add(group.groupIndex, group.parent);
                                            }
                                            else
                                            {
                                                if (group.haveTarget)
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        destGroup.groupIndex = -group.groupIndex - 100;//speciality layerIndex

                                                        file.index2Group.Add(destGroup.groupIndex, destGroup);
                                                    ...
                                                    group.parentIndex = destGroup.groupIndex;
                                                ...
                                                if (file.name2Group.ContainsKey(group.name))
                                                {
                                                    ...
                                                    file.index2Group.Remove(group.groupIndex);
                                                    group.groupIndex = readLayerIndex;
                                                    file.index2Group.Add(group.groupIndex, group);
                                                }
                                                else
                                                {
                                                    file.index2Group.Add(group.groupIndex, group);
                                        ...
                                        else if (layerType == 0 && !name.StartsWith("//"))
                                        {
                                            ...
                                            layer.layerIndex = readLayerIndex;
                                            ...
                                            if (layer.type == LayerType.Meta)
                                            {
                                                ...
                                                file.metaLayers.Add(layer.layerIndex, layer);
                                                file.index2Group[layer.parentIndex].metaLayers.Add(layer);
                                            }
                                            else
                                            {
                                                file.contentLayers.Add(layer.layerIndex, layer);
        ...
        private static int ReadDWord(this BinaryReader reader)
        {
            return (int)reader.ReadUInt32();
        }

        private static UInt16 ReadWord(this BinaryReader reader)
        {
            return reader.ReadUInt16();
        }

        private static string ReadUTF8(this BinaryReader reader)
        {
        ...
        private static Color[] ToColorBufferRGBA(byte[] bytes)
        {
            if (bytes.Length % 4 != 0)
            {
        ...
        private static byte[] ReadCompressedBytes(this BinaryReader reader, int count)
        {
            reader.ReadByte();
            reader.ReadByte();
        ...
        private static void _CheckMagicNumber<T>(T number, T expected)
        ...
        private static Exception _Error(string msg)
        {
            throw new Exception(msg);
        }

        private static void _Assert(bool expression, string msg)
        {
            if (!expression)
                _Error(msg);
        }
    ...
    public static class MetaLayerParser
    {
        ...
        private static readonly TokenType
        ...
        private static readonly TokenDefinition[] defs;
        ...
        private static void _Parse(Lexer lexer, Layer layer)
        {
        ...
        private static bool _SkipSpaces(Lexer lexer)
        {
            while (true)
            {
        ...
        private static string _Expect(Lexer lexer, TokenType tokenType)
        {
        ...
        private static Exception _ErrorEOF(Lexer lexer, params TokenType[] expected)
        {
            throw _Error(lexer, string.Format("Expected {0}, found EOF", _TokenTypeStr(expected)));
        }

        private static Exception _ErrorUnexpected(Lexer lexer, params TokenType[] expected)
        {
        ...
        private static Exception _Error(Lexer lexer, string msg)
        {
            throw new Exception(msg);
        }
    }
